...
namespace ACore.Server.Modules.AuditModule.Repositories.EF;

internal abstract class AuditSqlRepositoryImpl(DbContextOptions options, IACoreServerCurrentScope serverCurrentScope, ILogger<AuditSqlRepositoryImpl> logger)
  : EFContextBase(options, serverCurrentScope, logger), IAuditRepository
{
  ...
  private async Task<int> GetAuditUserIdAsync(string userId)
  {
    ...
    var cacheValue = await _serverCurrentScope.ServerCache.Get<AuditUserEntity>(keyCache); //await _mediator.Send(new MemoryCacheModuleGetQuery(keyCache));
    if (cacheValue != null)
    {
    ...
    await _serverCurrentScope.ServerCache.Set(keyCache, userEntity);
  ...
  private async Task<int> AuditTableId(string tableName, string? tableSchema, int version)
  {
    ...
    var cacheValue = await _serverCurrentScope.ServerCache.Get<AuditTableEntity?>(keyCache);
    if (cacheValue != null)
    {
    ...
    await _serverCurrentScope.ServerCache.Set(keyCache, tableEntity);
  ...
  private async Task<Dictionary<string, int>> AuditColumnId(int tableId, IEnumerable<EntityEventColumnItem> columns)
  {
    ...
    var cacheValue = await _serverCurrentScope.ServerCache.Get<Dictionary<string, int>>(keyCache);
    if (cacheValue != null)
    {
    ...
    _serverCurrentScope.ServerCache.Set(keyCache, res);
