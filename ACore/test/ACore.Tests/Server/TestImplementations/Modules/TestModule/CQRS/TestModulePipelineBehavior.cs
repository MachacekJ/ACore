  ...
  where TResponse : Result
{
  public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
  {
    ...
    if (!moduleBehaviorHelper.CheckIfModuleIsActive(testOptions.Value.TestModuleOptions, nameof(ACoreTestServiceExtensions.ContainerACoreTest), out var resultError))
